...
def image_directory(
    ...
    at the top of the stack of the slide indicated by slide_number
    of the extracted .pptx file directory pointed at by pptx_directory_path.
...
def image(
    ...
    """
    Appends the image pointed at by either image_path or image_name
    to the top of the stack of the slide indicated by slide_number
...
def main():
    ...
    parser.add_argument("-x", "--position-x", type=int, required=True, help=f"Image x position (from 0 up to {X_MAX}), measured in English Metric Units (EMU).")
    parser.add_argument("-y", "--position-y", type=int, required=True, help=f"Image y position (from 0 up to {Y_MAX}), measured in English Metric Units (EMU).")
    parser.add_argument("-cx", "--width", type=int, required=True, help="Image width, measured in English Metric Units (EMU).")
    parser.add_argument("-cy", "--height", type=int, help="Image height, measured in English Metric Units (EMU). If not provided, it is automatically calculated based on --width and the aspect ratio of the image.")
    parser.add_argument("-rot", "--rotation", type=int, default=0, help="Image rotation. If not provided or 0, no rotation is applied to the image.")
